...
public class BatchAlignPipeFast implements Serializable {
    ...
    private int readBinaryBlock(ReadInfo rInfo) {
        ...
        if (valueNX >= pAlign.bestNas) {
            return 1;
        }    // to many Ns and Xs in the read
    ...
    private String alignment(ReadInfo rInfo) {
        ...
        if (rInfo.readSize >= param.skipThreshold) {
            kmerSkip = 2;
        } // for longer than 1000, 2bps
...
// &end2(talign_info[4]);
        ...
        if ((param.chains == 0) || (param.chains == 2)) {
            ...
            for (i = 0; i < qGramSort.size(); i++) {
                if (reportRepeatHits > param.reportRepeatHits && param.reportRepeatHits != 0) {
                    break;
                }
    ...
    private int bandRevAlignment(Qgram qGram, int mRefBlockLen, int readLength) {
        ...
        for (i = 1; i < readLength; i++) {
            for (l = 0; l < bandWidth; l++) {
                ...
                if (pAlign.singleNtBitComplement[i] == pAlign.mRefBlockNtBit[j]) {
                    identityTemp++;
                }
    ...
    private int bandAlignment(Qgram qGram, int mRefBlockLen, int readLength) {
        ...
        for (i = 1; i < readLength; i++) {
            for (l = 0; l < bandWidth; l++) {
                ...
                if (pAlign.singleNtBit[i] == pAlign.mRefBlockNtBit[j]) {
                    identityTemp++;
                }
