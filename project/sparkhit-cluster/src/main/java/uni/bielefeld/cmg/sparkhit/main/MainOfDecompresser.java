...
import uni.bielefeld.cmg.sparkhit.util.ParameterForDecompresser;
 ...
 * Sparkhit-decompresser. It can also be specified at the input options
...
public class MainOfDecompresser {
    ...
    public static void main(String[] args){
        ...
        info.readParagraphedMessages("SparkHit Decompresser initiating ... \ninterpreting parameters.");
        info.screenDump();

        ParameterForDecompresser parameterD = null;
        try {
            parameterD = new ParameterForDecompresser(args);
        } catch (IOException e) {
            e.fillInStackTrace();
        } catch (ParseException e) {
            e.fillInStackTrace();
        }
        DefaultParam param = parameterD.importCommandLine();

        Pipelines pipes = new Pipelines();
        pipes.setParameter(param);
        pipes.sparkDecompresser();
